...
void Fun4All_SiliconOnlyAlignment(
	std::string const& segment_file = "segments.list",
	int                segment_line = 0, // 0-index
    int const          nEvents      = 1E3,
    std::string        outfilename  = "clusters_seeds",
    bool const         convertSeeds = true
) {
  ...
  for (ifstream segment_stream(segment_file); std::getline(segment_stream, input_file); --segment_line) {
	  if (!segment_line) break;
  }
  if (input_file.empty()) return;
  if (segment_line) return;
  ...
  outfilename += std::string{"_"} + std::to_string(runnumber) +  std::string{"_"} + std::to_string(segment);
 ...
 TRACKING::pp_mode = true;
  ...
  TString outfile = outfilename + ".root";
  ...
  if (G4TRACKING::convert_seeds_to_svtxtracks)
  {
    ...
    // SiliconTrackSeedContainer or TpcTrackSeedContainer or SvtxTrackSeedContainer
  ...
  resid->clusterTree();
  resid->noEventTree(); // Avoid segfault when running without TPC
  resid->linefitAll();
  //resid->hitTree();
  resid->zeroField();
  ...
  outfilename += "_";
  std::string hfbinstring = outfilename +   "helical_out.bin";
  std::string hfsteerstring = outfilename + "helical_steer.txt";
  std::string hfntpstring = outfilename +   "helical_ntuple.root";
  ...
  hf->set_intt_layer_fixed(3); // Fix 1/2 the INTT layers (1 of 2 barrels)
