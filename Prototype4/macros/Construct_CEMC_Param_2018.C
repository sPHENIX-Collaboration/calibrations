...
void Construct_CEMC_Param_2018()
{
  cout << "Construct_CEMC_Param() - Entry" << endl;

  gSystem->Load("libg4caloprototype.so");
  ...
  //  sector_map
  param->set_int_param("sector_map_size", 1);
  for (int sec = 0; sec < 1; ++sec)
  {
    const double rot = 2 * TMath::Pi() / (double) (azimuthal_n_sec) * ((double) (sec)) - TMath::Pi() / 2.;
...
//  for (int side = -1; side <= +1; side += 2)
  {
    ...
    // Dan: block 19-> 22 counting from 1
    const int start_block = 18;
//    const int last_block = 21;
    for (int block_id = start_block; block_id < start_block + ny; block_id++)
    {
      for (int block_sec = 0; block_sec < max_phi_bin_in_sec; block_sec++)
      {
        ...
        if (1)
        {
          ...
          AveragePlaneNorm *= (1 / AveragePlaneNorm.Mag());
