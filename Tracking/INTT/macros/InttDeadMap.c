...
void InttDeadMap()
{
	...
	InttDeadMap_Long_t N = 313100;
	InttDeadMap_Double_t frac = 0.02;

        struct InttDeadMap_Point_s* list = new struct InttDeadMap_Point_s[N];

        init_params(&params, list, N, 3.0, 3.0);
	...
	InttDeadMap_Double_t i_upper_ref = N - ceil(frac * N);
        ...
        std::cout << "\ti_upper: " << params.i_upper << "\t(" << i_upper_ref << ")\trel err: " << (params.i_upper - i_upper_ref) / i_upper_ref * 100 << "\%" << std::endl;
